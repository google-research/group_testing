...
class WetLab:
  ...
  def __init__(self,
               num_patients: int = 2,
               freeze_diseased: bool = True,
               base_infection_rate: Union[float, Iterable[float]] = 0.05,
               specificity: Union[float, Iterable[float]] = 0.97,
               sensitivity: Union[float, Iterable[float]] = 0.85):
...
class WetLabCSV(WetLab):
  ...
  def to_csv(self, groups: np.ndarray, marginals: np.ndarray):
